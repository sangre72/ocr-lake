"""오피스 문서(PPTX·HWP·DOCX) 관련 텔레그램 메시지 핸들러 (pdf_video_handlers.py 와 동일 패턴)"""
...
from core.docx.engine import UnsupportedDocxError, process_docx
...
def _is_docx_document(document) -> bool:
    mime = (document.mime_type or "").lower()
    name = (document.file_name or "").lower()
    return mime == _DOCX_MIME or name.endswith(".docx")
...
async def handle_docx(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = update.message
    document = message.document
    if not document or not _is_docx_document(document):
        return
    ...
    try:
        result = await process_docx(docx_bytes, lang=config.ocr_lang)
    except UnsupportedDocxError as exc:
        await message.reply_text(f"DOCX를 처리할 수 없습니다: {exc}")
        return
    except Exception:
        logger.exception("DOCX 처리 중 오류")
        await message.reply_text("DOCX 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.")
        return
    ...
    await message.reply_text(f"[DOCX 인식]\n추출된 텍스트:\n\n{reply}")
