"""Discord 첨부파일 → core 파이프라인 라우팅 (telegram_bot/handlers/* 와 대칭 패턴).

같은 입력(이미지/PDF/동영상/오피스문서)은 텔레그램·웹과 동일하게 core 의 같은 함수를 호출한다
...
def is_allowed(channel_id: int, allowed_channel_ids: list[int]) -> bool:
...
def _save_record_safely(*, route: str, extracted_text, description) -> None:
    """이력 저장 실패가 Discord 응답 흐름을 막지 않도록 격리(telegram_bot 패턴과 동일)."""
    try:
        save_record(
            source="discord",
            route=route,
        ...
        logger.exception("Discord OCR 이력 저장 실패(응답에는 영향 없음)")


async def process_attachment(filename: str, file_bytes: bytes, lang: str = "kor+eng") -> str:
    """첨부파일 확장자로 core 의 적절한 처리 함수를 호출하고, 사람이 읽을 응답 텍스트를 반환한다.

    Raises:
        ValueError: 지원하지 않는 포맷이거나 처리 중 오류(각 core 모듈의 Unsupported*Error 래핑)
    """
    name = filename.lower()
